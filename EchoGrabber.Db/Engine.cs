    ...
    public class Engine
    {
        ...
        public void LoadPodcasts(string url)
        {
            ...
            using (var conn = OpenConnection())
            {
                ...
                {
                    CommandText = "INSERT OR REPLACE INTO Podcasts (Title,Url,Archived) VALUES (@title,@url,@archived)"
                };
                cmd.Parameters.AddWithValue("@title", "");
                cmd.Parameters.AddWithValue("@url", "");
                cmd.Parameters.AddWithValue("@archived", url.IndexOf("archived") == -1 ? 0 : 1);
        ...
        public void LoadIssues(int podcastId)
        {
            ...
            using (var conn = OpenConnection())
            {
                ...
                insertCmd.CommandText = "INSERT OR IGNORE INTO Issues(Title,Url,DateTime,Duration,Guests,Size,PodcastId) " +
                "VALUES (@title,@url,@datetime,@duration,@guests,@size,@podcastId)";

                insertCmd.Parameters.Add("@title", DbType.String);
                insertCmd.Parameters.Add("@url", DbType.String);
                insertCmd.Parameters.Add("@datetime", DbType.DateTime);
                insertCmd.Parameters.Add("@duration", DbType.Single);
                insertCmd.Parameters.Add("@guests", DbType.String);
                insertCmd.Parameters.Add("@size", DbType.Single);
                insertCmd.Parameters.AddWithValue("@podcastId", podcastId);

                var getCmd = new SQLiteCommand(conn)
                {
                    CommandText = "SELECT Url FROM Podcasts WHERE Id=@podcastId LIMIT 1"
                };
                getCmd.Parameters.AddWithValue("@podcastId", podcastId);
                var url = getCmd.ExecuteScalar().ToString(); ;
                var issues = Grabber.GetAllPodcastLinks(url);
                ...
                foreach (var issue in issues)
                {
                    ...
                    insertCmd.Parameters["@duration"].Value = issue.Duration.TotalSeconds;
